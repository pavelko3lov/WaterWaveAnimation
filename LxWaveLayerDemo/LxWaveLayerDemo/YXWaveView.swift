...
open class YXWaveView: UIView {
    ...
    open var waveSpeed: CGFloat = 2.0
...
//        frame.size.height = 0
        realWaveLayer.frame = CGRect(x: 0, y: 0, width: frame.size.width, height: frame.size.height)
        maskWaveLayer.frame = CGRect(x: 0, y: 0, width: frame.size.width, height: frame.size.height)
    ...
    public convenience init(frame: CGRect, color:UIColor) {
        ...
        self.realWaveColor = color.withAlphaComponent(0.4)
    ...
    func wave() {
        ...
        for x in 0...Int(width) {
            ...
            path.addLine(to: CGPoint(x: CGFloat(x), y: y - vHeight))
            
            y = height * CGFloat(sinf( waveCurvature_f * Float(x) + offset_f + Float(phase) * Float(2 * M_PI)))
            maskpath.addLine(to: CGPoint(x: CGFloat(x), y: y - vHeight))
        }
